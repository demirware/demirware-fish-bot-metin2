...
class WindowActivationError(RuntimeError):
    """The target window could not be verified as the foreground window."""
...
class WindowManager:
    ...
    def activate_window(self, force_activate: bool = False):
        ...
        Raises WindowActivationError on failure, aborting the caller's input
        sequence. Callers must hold input_lock across activation and input.
        Uses ctypes directly instead of pygetwindow.activate() to avoid a GIL
        corruption bug in pygetwindow 0.0.9 on Python 3.13+."""
        if not self.selected_window:
            raise WindowActivationError("No target window selected; input cancelled")
        try:
            target_hwnd = self.selected_window._hWnd
            if not target_hwnd or not _user32.IsWindow(target_hwnd):
                self.invalidate_rect_cache()
                raise WindowActivationError("Target window is unavailable; input cancelled")
            ...
            # Never send input to whichever other window happens to be active.
            raise WindowActivationError(
                "Could not confirm target window focus after 3 attempts; input cancelled"
            )

        except WindowActivationError:
            raise
        except Exception as e:
            raise WindowActivationError(f"Window activation failed; input cancelled: {e}") from e
