...
try:
    from managed_bot import ManagedFishingBot as FishingBot  # noqa: E402
...
C = {
    ...
    "accent": "#60d5bc", "accent_deep": "#258f7e",
...
class TitleBar(QFrame):
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        self._title = QLabel("Demirware Fish Bot Metin2 · Geliştirme sürümü")
...
class DashboardTab(QWidget):
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        self.kpi_games = KPI("Toplam tur", "0", "tüm istemciler")
        ...
        # Game windows card
        windows_card = Card("Oyun istemcileri")
        ...
        reset_btn = QPushButton("  Yemi yeniledim")
        ...
        # Quick actions card
        qa_card = Card("Oturum kontrolleri", dot=False)
        qa_row = QHBoxLayout()
        qa_row.setSpacing(8)
    ...
    def setRunning(self, running: bool) -> None:
        ...
        if running:
            self.start_btn.setText("  BEKLET")
            self.start_btn.setIcon(make_icon("pause", 14, "#1a1108"))
            self.start_btn.setProperty("class", "BtnPrimary BtnLarge")
        else:
            self.start_btn.setText("  TÜMÜNÜ BAŞLAT")
            self.start_btn.setIcon(make_icon("play", 14, "#1a1108"))
            self.start_btn.setProperty("class", "BtnPrimary BtnLarge")
    ...
    def setPaused(self, paused: bool) -> None:
        self._paused = paused
        if paused:
            self.start_btn.setText("  DEVAM ET")
            self.start_btn.setIcon(make_icon("play", 14, "#1a1108"))
            self.start_btn.setProperty("class", "BtnPrimary BtnLarge")
        else:
            self.start_btn.setText("  BEKLET")
...
class WindowRow(QFrame):
    ...
    def __init__(self, index: int, parent: Optional[QWidget] = None):
        ...
        self.combo.addItem("— oyun penceresi seç —", "")
...
class SettingsTab(QWidget):
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        left = QVBoxLayout()
        left.setSpacing(10)
        self.classic_check = QCheckBox("Klasik balık sistemi")
    ...
    # Profiles are applied immediately after validation by the main window.
    def _profile_load(self) -> None:
        self.window().import_profile()

    def _profile_save(self) -> None:
        self.window().export_profile()
...
class FishbotWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.journal = EventJournal(Path(self.config_path).parent / "logs" / "session.jsonl")
        ...
        self._emergency_stop.connect(self.stop_all_bots)
    ...
    def _refresh_input_backend_pill(self) -> None:
        ...
        if name == "interception":
            self.header.setInputBackend(
                "INTERCEPTION",
                "Interception giriş sürücüsü etkin.",
            )
        else:
    ...
    def _default_config(self) -> dict:
        cfg = {
            "version": APP_VERSION,
            "operations": deepcopy(DEFAULT_OPERATIONS),
    ...
    def _build_footer(self) -> QFrame:
        ...
        wiz_btn = QPushButton("  Kurulum rehberi")
        ...
        self.jigsaw_launcher_btn = QPushButton("  Yapboz")
    ...
    def export_profile(self) -> None:
        self.save_config()
        path, _ = QFileDialog.getSaveFileName(self, "Profili kaydet", "fishbot-profil.json", "JSON (*.json)")
        if path:
            try:
    ...
    def import_profile(self) -> None:
        if self.bots:
            QMessageBox.information(self, "Profil", "Önce oturumları durdur.")
            return
        path, _ = QFileDialog.getOpenFileName(self, "Profil yükle", "", "JSON (*.json)")
        if not path:
            return
        try:
            ...
            # Handle bindings are intentionally not portable across machines.
            merged["selected_windows"] = [""] * MAX_WINDOWS
            self.config = merged
            for row in self.dashboard.window_rows:
            ...
            self.refresh_window_list()
            self.save_config()
            self.operations_tab.log.appendPlainText("Profil yüklendi; oyun pencerelerini yeniden seç.")
        except Exception as exc:
            QMessageBox.warning(self, "Profil yüklenemedi", str(exc))
    ...
    def load_config(self) -> None:
        ...
        try:
            from profile_validation import validate_profile
            saved = validate_profile(saved)
        except ValueError as exc:
            QMessageBox.warning(self, "Ayarlar yüklenemedi", str(exc))
            return
    ...
    def open_jigsaw_solver(self) -> None:
        ...
        self.dashboard.start_btn.setText("  TÜMÜNÜ BAŞLAT")
    ...
    def stop_all_bots(self, cancel_actions=True) -> None:
        ...
        if self.bots:
            self.header.setStatus("DURDURULUYOR", "idle")
            for i in self.bots:
                self.dashboard.window_rows[i].setStatus("warn", "DURUYOR")
        # Keep workers registered until their final callback. Starting a new
        # session while old threads still send input is not allowed.
    ...
    def _append_debug_status(self, msg: str) -> None:
        self.operations_tab.log.appendPlainText(msg)
    ...
    def _on_bot_stopped_ui(self, bot_id: int) -> None:
        ...
        if bot is not None and hasattr(bot, "metrics"):
            self.operations_tab.history[bot_id] = bot.metrics.snapshot(bot.total_games, bot.bait_counter)
